        ...
        #pragma omp parallel
        {
            ...
            for(int rep = 0; rep < nrep; rep++) {
                ...
                for(int step = 0; step < timescale; step++) {
                    ...
                    std::vector<cell_update> private_updates;
                                ...
                                if(randval < land_mask[i][j]) {
                                    ...
                                    if(randval <= inv_sum/8) {
                                        ...
                                        if(neighborhood[inv_index] != land_grid[i][j]) {
                                            private_updates.push_back({i, j, neighborhood[inv_index]});
                                        }
                                    }
                                }
                            }
                    ...
                    #pragma omp critical
                    updates.insert(updates.end(), private_updates.begin(), private_updates.end());
                    ...
                    if( std::numeric_limits<cell_type>::max() / (1+mutsize) < max ) {
                        float tmp_sum = 0;
                        #pragma omp for private(tmp_sum) reduction(+:land_grid_mean)
                        for(int i = 0; i < size; i++) {
                            for(int j = 0; j < size; j++) {
